...
p("Nove termos sustentam todo o restante do documento: os três primeiros dizem quem trabalha, os "
  "três seguintes explicam como o trabalho é cobrado, e os três últimos só reaparecem nas seções 8 "
  "e 9 — mas ficam definidos aqui, para que aquelas páginas não dependam de nenhuma palavra nova.",
  depois=3)
...
tabela(["Termo", "O que significa aqui"],
       [["*Modelo de linguagem",
        ...
        ["*Prefixo",
         "o começo da requisição — a parte que se repete igual de uma volta para a outra"],
        ["*Reaproveitamento",
...
p("Três fatos encadeados explicam o custo do sistema — e o terceiro é o que dá a alavanca.",
  depois=2)
...
marcador("Logo, o texto de abertura é recobrado em toda volta. ", "As regras do sistema e o índice "
         "do projeto vão junto sempre. Num despacho de quinze voltas, paga-se quinze vezes pelo "
         "mesmo texto.")
...
p("Para que isso funcione, a requisição é montada do conteúdo que nunca muda para o que muda "
  "sempre. É essa ordem que a figura abaixo mostra.", antes=2)

figura("fig5-cache-v6.png", "Figura 8 — Os quatro primeiros blocos podem ser guardados e "
...
caixa("A CONTA, EM NÚMEROS REDONDOS",
      "Suponha uma abertura fixa de 20 mil tokens (as regras mais o índice do projeto) e um despacho "
      "de quinze voltas. Sem reaproveitamento, são 15 × 20 mil = 300 mil tokens cobrados a preço "
      "cheio. Com reaproveitamento: 20 mil pagos uma vez com 25% de acréscimo, mais catorze voltas "
      "a um décimo do preço — o equivalente a cerca de 53 mil. Aproximadamente um sexto da conta, "
      "pelo mesmo trabalho.")

tabela(["Alavanca de custo", "Efeito"],
       [["Começo da requisição sempre igual", "o trecho repetido cai para um décimo do preço"],
        ["Cada papel recebe só o que usa", "quem revisa recebe as mudanças, não o código inteiro"],
        ["Checagens por comando, antes do modelo", "o que falha volta sem gastar um agente"],
...
caixa("A ARMADILHA QUE ANULA ISTO, EM SILÊNCIO",
      ...
      "sistema traz um teste que monta esse começo duas vezes e falha se os dois resultados "
      "divergirem.",
...
p("Um agente começa sempre sem saber nada do projeto. Deixá-lo abrir arquivo por arquivo até "
  "entender seria o pior caminho possível, porque cada leitura é uma volta — e volta é exatamente "
  "o que custa. Então o sistema entrega o conhecimento pronto, em duas camadas, porque existem duas "
  "perguntas diferentes.")

marcador("“O que existe e como se chama?” ", "Responde um índice gerado por script: a árvore de "
         "arquivos com a assinatura de cada função e uma linha dizendo para que serve. Nenhum modelo "
         "envolvido, custo zero, sempre atualizado. É o sumário do projeto, e vai inteiro no prompt.")
...
figura("fig7-rag-v6.png", "Figura 9 — A indexação roda ao commitar, fora do caminho do trabalho; a "
